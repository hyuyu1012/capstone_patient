...
class YamnetClassifier {
  ...
  List<MapEntry<String, double>> classify(Float32List samples) {
    return classifyIndexed(samples)
        .map((r) => MapEntry(r.name, r.score))
        .toList();
  }

  /// classify()와 동일하되 클래스 인덱스를 함께 반환.
  /// MedicationScorer.addYamnetResult(index, confidence)에 바로 연결 가능.
  List<YamnetResult> classifyIndexed(Float32List samples) {
    ...
    for (int c = 0; c < numClasses && c < _labels.length; c++) {
      results.add(YamnetResult(index: c, name: _labels[c], score: avgScores[c]));
    }
...
/// YAMNet 분류 결과 단일 항목 (인덱스 + 이름 + 스코어).
class YamnetResult {
  final int index;
  final String name;
  final double score;
  const YamnetResult({required this.index, required this.name, required this.score});
  MapEntry<String, double> toEntry() => MapEntry(name, score);
}
